...
class ImageModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_ugc_new.image';
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_ugc_new.edit.image.data', array());
	...
	public function duplicate(&$pks)
	{
		...
		foreach ($pks as $pk)
		{
				...
				if (!empty($table->review_id))
				{
					if (is_array($table->review_id))
					{
						$table->review_id = implode(',', $table->review_id);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__ugc_images');
				$max             = $db->loadResult();
